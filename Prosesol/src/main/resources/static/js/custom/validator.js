				...
				function(e) {
					...
					if ($('.rfcText').val()) {
						...
						if (inputLength < rfcLength) {
							$('.rfcText')
									.after(
											"<span class='validation' style='color:red;margin-bottom: 20px;'>El RFC no cumple con la longitud correcta</span>");
							e.preventDefault();
						}
						if (!focusSet) {
							$('.rfcText').focus();
					...
					if ($('.nssText').val()) {
							...
							if (inputLength < nssLength) {
								$('.nssText')
										.after(
												"<span class='validation' style='color:red;margin-bottom: 20px;'>El NSS no cumple con la longitud correcta</span>");
								e.preventDefault();
							} else if (!$.isNumeric($('.nssText').val())) {
								$('.nssText')
										.after(
												"<span class='validation' style='color:red;margin-bottom: 20px;'>Proporcione solo números</span>");
								e.preventDefault();
							}
							if (!focusSet) {
								$('.nssText').focus();
					...
					if ($('.curpText').val().length > 0) {
						...
						if (inputLength < curpLength) {
							$('.curpText')
									.after(
											"<span class='validation' style='color:red;margin-bottom: 20px;'>El curp no cumple con la longitud correcta</span>");
							e.preventDefault();
						}
